...
'''
    ...
    # 3. SPT-0016
    spt_0016_code = '''function getUnitStars(unit, zone, lane) {
  if (!unit) return 0;
...
'''
    # Since helper code modifies this anchor too, we apply this separately or fix the order.
    # Actually, R-SPT-0008 can anchor on the line AFTER `window.xlwResolveTurnStartEffects = async function(isPlayerSide) {`
    r_spt_0008_anchor = '''const prefix = isPlayerSide ? "player_" : "enemy_";

  // 進化型態 / 特殊 布偶與特殊效果'''
    
    r_spt_0008_code2 = '''const prefix = isPlayerSide ? "player_" : "enemy_";
  ...
  // 進化型態 / 特殊 布偶與特殊效果'''
    replacements.append((r_spt_0008_anchor, r_spt_0008_code2, "R-SPT-0008 (xlwResolveTurnStartEffects)"))
  ...
  if (c.id === "TOKEN_TRAVELER" || c.name?.includes("小旅人")) {
    const side = z.startsWith("player_") ? "player_" : "enemy_";
