...
destroy_injection = """
  ...
  // Trigger when a friendly fairy dies. 
  // Normally Traps are not heavily supported natively so we check hand for now, or if we assume it's played from hand immediately?
  // Wait, EVL-0019 is a magic card, maybe it's in the field (equipments) or in the magic zone.
  // Actually, standard trap logic in this game checks if it's placed somewhere, but let's just assume it checks hand or maybe magic field? 
  // The plan said "魔法/陷阱卡。當己方妖精單位被戰鬥或效果破壞時觸發". Let's check `magic_zone`? But there is no magic zone, only equip. 
  // Oh, wait, in XingLingWang there's no trap zone, magic cards are either equipped or immediate. 
  // Let's make it trigger from HAND for now.
  if (isFairy(unit)) {
    const isPlayer = zone.startsWith("player_");
    const myHand = isPlayer ? hand : (window.XLW_ENEMY?.hand || []);
...
summon_marker = '      if (card.id === "SSSR-FMS-0063" || card.name?.includes("特製壽司")) {'
summon_injection = """
        // 妖精女王 梅瑟琳 (EVL-0002)
        ...
        if (card.id === "EVL-0015" || card.name?.includes("妖精預言家")) {
          ...
          if (myDeck.length >= 3 && isPlayer) {
            ...
            if (confirmProphet) {
              ...
              // Simply prompt to select which one is placed at the top (idx 2), middle (idx 1), bottom (idx 0).
              // Since custom drag-drop is complex, we will ask which card goes on TOP, then which goes on MIDDLE.
...
if 'EVL-0002' not in text:
    if summon_marker in text:
        text = text.replace(summon_marker, summon_injection + summon_marker)
    else:
        # Fallback if marker not found: insert at the end of performSummonToSlot 
        # (just before "if (isMultiplayer) {")
        fallback_marker = '      if (isMultiplayer) {'
        if fallback_marker in text:
            text = text.replace(fallback_marker, summon_injection + fallback_marker)
