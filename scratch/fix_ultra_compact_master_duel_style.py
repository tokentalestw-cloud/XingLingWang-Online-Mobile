...
def fix_ultra_compact():
    ...
    # 1. Update static/game_v8.js adjustBoardScale for maximum battlefield scale(0.48)
...
function adjustBoardScale() {
  ...
  if (isMobile && isLandscapeMobile) {
    ...
    ultra_compact_css = """/* ==========================================================================
   REAL MOBILE DEVICE LANDSCAPE RWD (極致微縮置底 3D 懸浮手牌與兩側極簡化排版)
   ========================================================================== */
...
@media (max-width: 1024px) and (orientation: landscape) {
  ...
  /* 2. 對戰棋盤放大為 scale(0.46) 絕對置中，極大化戰場視野 */
  .board-wrap {
    width: 100% !important;
  ...
  /* 3. 我方得分看板 (極簡圓形霓虹數字，隱藏文字，直徑僅 40px) -> 靠左下角 */
  ...
  /* 4. 對手得分看板 (極簡圓形霓虹數字，隱藏文字，直徑僅 40px) -> 靠右上角 */
  ...
  /* 5. 對手狀態欄 (#xlwEnemyInfoPanel) 大幅微縮為 42% 並靠左側，分數板下方 */
  #xlwEnemyInfoPanel {
    position: absolute !important;
    left: 12px !important;
    top: 80px !important;
  ...
  /* 6. 右側操作按鈕面板 (#stableActionPanel) 極致縮小為 0.44 倍並靠右下角，完全不擋格子 */
  #stableActionPanel {
    position: absolute !important;
    right: 12px !important;
    bottom: 12px !important;
...
"""
    ...
    css_content += ultra_compact_css
    open(css_path, 'w', encoding='utf-8').write(css_content)
    print("2. Updated static/style_v8.css with ultra compact Master Duel style successfully!")

    # Update cache-buster in static/index.html to v=16.10-master-duel-ultra-compact
    idx_content = open(idx_path, encoding='utf-8').read()
    idx_content = re.sub(r'game_v8\.js\?v=[^"\']+', 'game_v8.js?v=16.10-master-duel-ultra-compact', idx_content)
    idx_content = re.sub(r'style_v8\.css\?v=[^"\']+', 'style_v8.css?v=16.10-master-duel-ultra-compact', idx_content)
