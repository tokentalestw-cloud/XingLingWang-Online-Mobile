...
for idx, line in enumerate(lines):
    if "sync_game_state" in line.lower() or "sendfullgamestate" in line.lower():
        ...
        # if it's a function definition, print next 25 lines
        if "function " in line:
            for i in range(idx+1, min(idx+35, len(lines))):
                out.append(f"  {lines[i].rstrip()}")

with open("c:/Users/a2132/Documents/星靈王/XingLingWang_v7_fixed/scratch/scratch_sync_fns.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))
