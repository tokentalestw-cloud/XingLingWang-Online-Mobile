...
for idx, line in enumerate(lines):
    if "function destroyunit(" in line.lower():
        out.append(f"L{idx+1}: {line.strip()}")
        # print next 40 lines
        for i in range(idx+1, min(idx+45, len(lines))):
            out.append(f"  {lines[i].rstrip()}")

with open("c:/Users/a2132/Documents/星靈王/XingLingWang_v7_fixed/scratch/scratch_destroy_unit.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(out))
