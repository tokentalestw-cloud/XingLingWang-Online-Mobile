...
def fix_museum():
    ...
    # 1. Update xlwShowSpellActivationOverlay & xlwShowTributeSummonOverlay in static/game_v8.js
    js_path = 'static/game_v8.js'
    js_content = open(js_path, encoding='utf-8').read()

    new_overlay_fns = """function xlwShowSpellActivationOverlay(card, side) {
  return new Promise((resolve) => {
    ...
    // 保底強制 Fixed 全螢幕居中樣式，防止掉到頁面最下方爆版
...
function xlwShowTributeSummonOverlay(card, side) {
  return new Promise((resolve) => {
    ...
    if o_start >= 0 and o_end >= 0:
        js_content = js_content[:o_start] + new_overlay_fns + "\n\n" + js_content[o_end:]
        open(js_path, 'w', encoding='utf-8').write(js_content)
        print("1. Updated xlwShowSpellActivationOverlay in static/game_v8.js successfully!")

    # 2. Append CSS rules for spell & tribute overlays to static/style_v8.css
...
/* ==========================================================================
   SPELL & TRIBUTE ACTIVATION OVERLAYS (全螢幕精緻動畫彈窗居中)
   ========================================================================== */
...
"""
    ...
    print("2. Appended overlay CSS rules to static/style_v8.css successfully!")
