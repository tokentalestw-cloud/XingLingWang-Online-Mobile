...
def fix_final_priority():
    ...
    # 1. Update static/game_v8.js scaling logic to use scale(0.44) and position correctly
...
function adjustBoardScale() {
  ...
  if (isMobile && isLandscapeMobile) {
    ...
    # 2. Update static/style_v8.css with strict media query orientation filters and final priority block
    css_content = open(css_path, encoding='utf-8').read()

    # Isolate all standard max-width: 900px queries to only match portrait mode to prevent landscape conflicts
    css_content = re.sub(r'@media\s*\(max-width:\s*900px\)\s*\{', '@media (max-width: 900px) and (orientation: portrait) {', css_content)

    # Clean any old RWD or Master Duel css blocks so we can append a clean priority block at the bottom
    block_marker_1 = "/* ==========================================================================\n   REAL MOBILE DEVICE LANDSCAPE RWD"
    if block_marker_1 in css_content:
        css_content = css_content[:css_content.find(block_marker_1)]
        
    block_marker_2 = "/* ==========================================================================\n   MOBILE & IPHONE 14 HAND CARD"
    if block_marker_2 in css_content:
        css_content = css_content[:css_content.find(block_marker_2)]

    final_master_duel_css = """/* ==========================================================================
   REAL MOBILE DEVICE LANDSCAPE RWD (頂級優先權：懸浮立體手牌、兩側微縮極小按鈕)
   ========================================================================== */
...
@media (max-width: 1024px) and (orientation: landscape) {
  ...
  /* 1. 最上排工具列 (Top Bar) 大幅縮放為 0.35 倍，且靠左對齊，寬度絕不超出畫面 */
  .topbar-grouped-v9 {
  ...
  /* 2. 對戰棋盤放大為 scale(0.44) 絕對置中，極大化戰場視野 */
  .board-wrap {
    width: 100% !important;
  ...
  .score-badge-fixed.score-p1 {
    ...
    box-shadow: 0 0 12px rgba(82, 196, 26, 0.5) !important;
  ...
  .score-badge-fixed.score-p2 {
    ...
    align-items: center !important;
    justify-content: center !important;
    box-shadow: 0 0 12px rgba(255, 77, 79, 0.5) !important;
    transform: none !important;
    margin: 0 !important;
  }
  ...
  /* 5. 對手狀態欄 (#xlwEnemyInfoPanel) 縮小為 45% 並靠左側，分數板下方 */
  #xlwEnemyInfoPanel {
    position: absolute !important;
    left: 12px !important;
    top: 80px !important;
  ...
  /* 6. 右側操作按鈕面板 (#stableActionPanel) 極致縮小為 0.52 倍並靠右下角 */
  #stableActionPanel {
    position: absolute !important;
    right: 12px !important;
    bottom: 12px !important;
  ...
  /* 8. 真正懸浮立體手牌專區 (無背景板、無文字、100% 懸浮透底) */
  .hand-panel {
    position: absolute !important;
  ...
  /* 3D 視角立體斜度手牌 (perspective + rotateX) */
  .hand .card {
...
"""
    ...
    css_content += final_master_duel_css
    open(css_path, 'w', encoding='utf-8').write(css_content)
    print("2. Updated static/style_v8.css with final priority Master Duel CSS successfully!")

    # Update cache-buster in static/index.html to v=16.00-master-duel-priority-fixed
    idx_content = open(idx_path, encoding='utf-8').read()
    idx_content = re.sub(r'game_v8\.js\?v=[^"\']+', 'game_v8.js?v=16.00-master-duel-priority-fixed', idx_content)
    idx_content = re.sub(r'style_v8\.css\?v=[^"\']+', 'style_v8.css?v=16.00-master-duel-priority-fixed', idx_content)
